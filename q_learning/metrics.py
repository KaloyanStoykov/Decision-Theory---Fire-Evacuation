...
class Metrics:
    ...
    def save(self):
        ...
        plt.legend()
        plt.savefig(
            f"metrics/metrics.dr={config.distance_reward}.tsp={config.time_step_punishment}.N={N_EPISODES}.im={config.illeagal_move_punishment}.fer={config.fire_extinguished_reward}.df={config.discount_factor}.png"
        )
        plt.close()
